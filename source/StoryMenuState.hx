...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		for (char in 0...3)
		{
			var daChar:String = weekCharacters[curWeek][char];

			if (daChar != '')
			{
				var weekCharacterThing:MenuCharacter = new MenuCharacter((FlxG.width * 0.25) * (1 + char) - 150, daChar);
	...
	function updateText()
	{
		...
		for (i in 0...3)
		{
			if (weekCharacters[curWeek][i] != '')
			{
				grpWeekCharacters.members[i - substract].animation.play(weekCharacters[curWeek][i]);
			}
			else substract++;
		}
